...
public class GenericsExample {
    ...
    public static void main(String[] args) {
        // 수정된 부분: Recipe 클래스를 생성할 때 Flour 대신 Egg를 사용하도록 수정
        Recipe<Ingredient, Cake> recipe = new Recipe<>(new Egg()) {
            @Override
            public Cake cook() {
                // 수정된 부분: Ingredient의 mix() 메서드를 호출할 수 있도록 수정
                getIngredient().mix();
                return new Cake();
            }
        };
    ...
    abstract static class Recipe<T extends Ingredient, R> { // 수정된 부분: T를 Ingredient로 제한
